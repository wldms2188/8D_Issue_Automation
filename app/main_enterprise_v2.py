"""Enterprise UI v2: compact guided inputs, balanced focus panels and visible workflow progress."""
...
class EnterpriseAppV2(ent.EnterpriseApp):
    ...
    def _find_section_card(self, number):
        ...
        w=found[0]
        # number label -> title row -> white section card
        for _ in range(3):
            if w.master is self: break
            w=w.master
            try:
                if w.cget('bg')=='white' and int(w.cget('highlightthickness'))==1:return w
            except Exception: pass
        return w

    def _balance_and_focus_cards(self):
        """Default 50:50. Clicking/focusing a card enlarges it; clicking outside restores 50:50."""
        self.card01=self._find_section_card('01'); self.card02=self._find_section_card('02')
        if not self.card01 or not self.card02:return
        parent=self.card01.master
        if self.card02.master is not parent:return
        try:
            self.card01.pack_forget(); self.card02.pack_forget()
            parent.grid_columnconfigure(0,weight=1,uniform='half'); parent.grid_columnconfigure(1,weight=1,uniform='half'); parent.grid_rowconfigure(0,weight=1)
            self.card01.grid(row=0,column=0,sticky='nsew',padx=(0,7)); self.card02.grid(row=0,column=1,sticky='nsew',padx=(7,0))
        except Exception:return
        self._cards_parent=parent
        def bind_tree(w,side):
            w.bind('<Button-1>',lambda _e,s=side:self._expand_card(s),add='+')
            w.bind('<FocusIn>',lambda _e,s=side:self._expand_card(s),add='+')
            for c in w.winfo_children():bind_tree(c,side)
        bind_tree(self.card01,1); bind_tree(self.card02,2)
        self.bind('<Button-1>',self._maybe_reset_cards,add='+')
    ...
    def _set_card_ratio(self,active=0):
        ...
        try:
            if active==1:
                p.grid_columnconfigure(0,weight=13,uniform=''); p.grid_columnconfigure(1,weight=7,uniform='')
            elif active==2:
                p.grid_columnconfigure(0,weight=7,uniform=''); p.grid_columnconfigure(1,weight=13,uniform='')
            else:
    ...
    def _maybe_reset_cards(self,event):
        w=event.widget
        def inside(child,parent):
            while child is not None:
                if child is parent:return True
                try: child=child.master
                except Exception:return False
            return False
        if not inside(w,getattr(self,'card01',None)) and not inside(w,getattr(self,'card02',None)): self._set_card_ratio(0)

    def _install_progress_panel(self):
        """Single-line progress strip so the filled bar cannot be clipped below the visible result area."""
        parent=self.log.master
        self.progress_value=tk.DoubleVar(value=0); self.progress_text=tk.StringVar(value='0% · 실행 대기')
        panel=tk.Frame(parent,bg='white',height=28); panel.pack(fill='x',padx=16,pady=(0,6),before=self.log); panel.pack_propagate(False)
        tk.Label(panel,text='진행률',bg='white',fg=ui.MUTED,font=('Malgun Gothic',8,'bold'),width=7,anchor='w').pack(side='left')
        self.progress_canvas=tk.Canvas(panel,height=10,bg='#DDE5EB',highlightthickness=1,highlightbackground='#CBD6DE',bd=0)
        self.progress_canvas.pack(side='left',fill='x',expand=True,padx=(4,10),pady=8)
        self.progress_fill=self.progress_canvas.create_rectangle(0,0,0,10,fill='#1769AA',outline='')
        tk.Label(panel,textvariable=self.progress_text,bg='white',fg=ui.NAVY,font=('Malgun Gothic',8,'bold'),width=25,anchor='e').pack(side='right')
    ...
    def _paint_progress_bar(self):
        try:
            width=max(1,self.progress_canvas.winfo_width()); height=max(1,self.progress_canvas.winfo_height()); pct=max(0,min(100,float(self.progress_value.get())))
            self.progress_canvas.coords(self.progress_fill,0,0,width*pct/100,height); self.progress_canvas.tag_raise(self.progress_fill)
        except Exception:pass
    ...
    def _sync_progress_from_status(self,*_):
        ...
        self.progress_value.set(0 if pct==0 else max(float(self.progress_value.get()),pct)); self.progress_text.set(f'{int(self.progress_value.get())}% · {label}'); self._paint_progress_bar()
