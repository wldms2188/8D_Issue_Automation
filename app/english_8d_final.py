...
def _cell_in_expected_d_region(box,sec,regions):
    ...
    if not n or not regions:return True
...
def _scoped_table_semantic_fields(path):
    ...
    def add(key,text):
        text=_norm_line(text)
        if text and not _is_photo_caption(text):buckets[key].append(text)
        ...
        for sh in sl.shapes:
            ...
            for r,row in enumerate(tb.rows):
                ...
                for cc,raw in enumerate(cells):
                    ...
                    # Extra named rows inside the accepted 4D table/region are kept
                    # under occurrence cause, except standard escape/system headings.
                    if sec=='4D':
