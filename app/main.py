...
SECTION_ALIASES = {
    '2d': ['2D 현상', '2D현상'],
    '3d': ['3D 임시대책', '3D 임시 대책', '3D임시대책', '임시대책', '임시 대책'],
    '4d': ['4D 원인분석', '4D 원인 분석', '4D원인분석', '발생 원인'],
    '5d': ['5D 개선대책', '5D 개선 대책', '5D개선대책', '개선 대책'],
    '6d': ['6D 유효성점검', '6D 유효성 점검', '6D유효성점검', '효과검증', '효과 검증'],
    '7d': ['7D 수평전개', '7D 수평 전개', '7D수평전개', '수평전개', '수평 전개', '재발방지', '재발 방지'],
}
def norm(x):
    if x is None: return ''
...
def blank(x):
    x = norm(x)
    return not x or x in ['000', '00', '‘00.00.00', '00호기', '담당자/ 반영일', '유첨 5why', '대책에 대한 효과가 검증되어', '10개 Lot 모니터링등', '본 이슈로 인한 비용 및 산출 근거']
def key(x): return re.sub(r'[\s_/\\\-:.,·]+', '', one(x).lower())
def cell(tb, r, c): return norm(tb.cell(r, c).text) if 0 <= r < len(tb.rows) and 0 <= c < len(tb.columns) else ''
def near(tb, labels):
    ...
    for r, row in enumerate(tb.rows):
        for c, cl in enumerate(row.cells):
            ...
            if raw and any(x in low for x in labels):
                ...
                for v in cand:
                    if v and not blank(v) and not any(norm(v).lower() == x for x in labels): return v
    return ''
def parse_year_month(text):
    s = one(text)
    m = re.search(r'(20\d{2})\s*[-./]\s*(\d{1,2})', s)
    if m: return m.group(1), m.group(2)
    m = re.search(r'(?<!\d)(\d{2})\s*[-./]\s*(\d{1,2})\s*[-./]\s*(\d{1,2})(?!\d)', s)
    if m: return '20' + m.group(1), m.group(2)
    m = re.search(r'(?<!\d)(\d{4})(\d{2})(\d{2})(?!\d)', s)
    if m: return m.group(1), m.group(2)
    return '', ''
def extract(p):
    prs = Presentation(p)
    keys = ['issue_name', 'task_name', 'customer', 'occurrence_site', 'occurrence_date', 'receipt_date', 'problem', 'temporary_action', 'customer_response', 'cause_4d', 'leak_cause', 'system_cause', 'action_5d', 'verification_6d', 'spread_7d']
    d = {k: '' for k in keys}; d['standards'] = {x: '' for x in STD}; texts = []
    mp = {'task_name': ['과제명'], 'customer': ['Customer', '고객사'], 'occurrence_site': ['발생 Site'], 'occurrence_date': ['발생 일자'], 'receipt_date': ['접수 일자'], 'problem': ['불량 현상', '문제 현황'], 'temporary_action': ['임시 조치'], 'customer_response': ['고객 대응'], 'cause_4d': ['발생 원인'], 'leak_cause': ['유출 원인'], 'system_cause': ['IT 시스템'], 'action_5d': ['개선 대책', '대책'], 'verification_6d': ['효과검증', '효과 검증', '유효성 점검', '유효성점검'], 'spread_7d': ['수평전개', '수평 전개', '재발방지', '재발 방지']}
    for sl in prs.slides:
        for sh in sl.shapes:
            ...
            if getattr(sh, 'has_table', False):
                tb = sh.table
                for k, labels in mp.items(): d[k] = d[k] or near(tb, labels)
                for s in STD: d['standards'][s] = d['standards'][s] or near(tb, [s])
    full = '\n'.join(texts)
    fallback = {'issue_name': ['이슈명'], 'task_name': ['과제명'], 'customer': ['Customer', '고객사'], 'occurrence_site': ['발생 Site'], 'problem': ['불량 현상'], 'cause_4d': ['발생 원인'], 'action_5d': ['개선 대책'], 'verification_6d': ['효과검증', '효과 검증'], 'spread_7d': ['수평전개', '수평 전개']}
    for k, labels in fallback.items():
        if not d[k]:
...
def write_row(ws,r,d,g):
    y,m=parse_year_month(d.get('occurrence_date')); st=status(d)
    vals={2:datetime.date.today(),3:g.get('plm_no',''),4:g.get('form_factor',''),5:g.get('product_type',''),6:y,7:m,8:g.get('team',''),9:g.get('owner',''),10:task(d),11:g.get('sample',''),12:g.get('stage',''),13:d.get('occurrence_site',''),14:one(d.get('problem'),120),15:'',16:one(' / '.join(x for x in [d.get('cause_4d'),d.get('leak_cause'),d.get('system_cause')] if x),160),17:one(d.get('action_5d'),160),18:st}
    for c,v in vals.items():ws.cell(r,c).value=v
    return st
def save_excel(wb,out):
    try: wb.save(out); return out
    except PermissionError:
        p=Path(out); alt=p.with_name(p.stem+'_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S')+p.suffix); wb.save(alt); return alt
def update_excel(src,out,d,g,new=False):
    ...
    Path(out).parent.mkdir(exist_ok=True);saved=save_excel(wb,out)
    if str(saved)!=str(out):msg+=f'\n※ 기존 결과 파일이 열려 있어 새 파일로 저장: {saved.name}'
    return msg,saved
def signal(c,s):
    c.text='●';rgb=SIGNAL[s]
...
def replace_metadata(sl,t,i,g,d):
    for sh in sl.shapes:
        if hasattr(sh,'text') and sh.text:
            old=sh.text;new=old
            if '과제명_이슈 제목' in old:new=old.replace('과제명_이슈 제목',f'{t}_{i}')
            if '이슈명' in new and ':' in new:
                prefix=new.split(':',1)[0]
                if prefix.strip().startswith('이슈명'):new=prefix+' : '+i
            if '00팀 담당자 : 000' in new:new=new.replace('00팀 담당자 : 000',f"{g.get('team','')} 담당자 : {g.get('owner','')}")
            if new!=old:sh.text=new
    for tb,r,c,cl in table_cells(sl):
        ...
        if '이슈명' in new and ':' in new:
            ...
            if prefix.strip().startswith('이슈명'):new=prefix+' : '+i
        if '00팀 담당자 : 000' in new:new=new.replace('00팀 담당자 : 000',f"{g.get('team','')} 담당자 : {g.get('owner','')}")
        if old!=new:cl.text=new
def section_content(d):
    return {'2d':one(d.get('problem'),700) or '내용 미기재','3d':one(d.get('temporary_action'),500) or '내용 미기재','4d':one(' / '.join(x for x in [d.get('cause_4d'),d.get('leak_cause'),d.get('system_cause')] if x),700) or '내용 미기재','5d':one(d.get('action_5d'),700) or '내용 미기재','6d':one(d.get('verification_6d'),700) or '내용 미기재','7d':one(d.get('spread_7d'),700) or '내용 미기재'}
def section_key(text):
    s=norm(text).lower()
    for k,aliases in SECTION_ALIASES.items():
        if any(a.lower() in s for a in aliases):return k
    return None
def is_section_label(text):return section_key(text) is not None
def find_blank_right(tb,r,c,used):
    for cc in range(c+1,len(tb.columns)):
        cl=tb.cell(r,cc);txt=norm(cl.text)
        if id(cl) in used or is_section_label(txt):continue
        if blank(txt) or txt in ['내용','입력','내용 입력','000','예시']:return cl
    return None
def fill_table_sections(tb,contents,used):
    found=set();labels=[]
    for r in range(len(tb.rows)):
        for c in range(len(tb.columns)):
            txt=norm(tb.cell(r,c).text);k=section_key(txt)
            if k:labels.append((k,r,c,tb.cell(r,c),txt))
    for k,r,c,label_cell,label_text in labels:
        content=contents[k];dest=find_blank_right(tb,r,c,used)
        if dest is not None:dest.text=content;used.add(id(dest));found.add(k)
        else:
            prefix=label_text.split(':',1)[0].strip();label_cell.text=prefix+'\n'+content;found.add(k)
    return found
def fill_shape_sections(sl,contents):
    shapes=list(sl.shapes);used=set();found=set()
    for sh in shapes:
        if not hasattr(sh,'text') or not norm(sh.text):continue
        k=section_key(sh.text)
        if not k:continue
        best=None;best_dist=None
        for cand in shapes:
            if cand is sh or id(cand) in used or not hasattr(cand,'text'):continue
            if norm(cand.text) or getattr(cand,'has_table',False):continue
            if cand.left < sh.left+sh.width:continue
            vertical_overlap=min(sh.top+sh.height,cand.top+cand.height)-max(sh.top,cand.top)
            if vertical_overlap<=0:continue
            dist=cand.left-(sh.left+sh.width)
            if best is None or dist<best_dist:best,best_dist=cand,dist
        if best is not None:best.text=contents[k];used.add(id(best))
        else:sh.text=norm(sh.text).split(':',1)[0]+': '+contents[k]
        found.add(k)
    return found
def fill_8d_sections(target,d):
    contents=section_content(d);found=set();tables=[]
    for tb,_,_,_ in table_cells(target):
        if id(tb) not in [id(x) for x in tables]:tables.append(tb)
    for tb in tables:found|=fill_table_sections(tb,contents,set())
    found|=fill_shape_sections(target,contents)
    return found
def save_ppt(prs,out):
    try:prs.save(out);return out
    except PermissionError:
        p=Path(out);alt=p.with_name(p.stem+'_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S')+p.suffix);prs.save(alt);return alt
def weekly(src,out,d,g,mode):
    prs=Presentation(src);st=status(d);t=task(d);i=one(d.get('issue_name'));summary=None
    for sl in prs.slides:
        for sh in sl.shapes:
            if getattr(sh,'has_table',False) and sh.table.rows:
                header=' '.join(cell(sh.table,0,c) for c in range(len(sh.table.columns)))
    ...
    if summary:
        ...
        vals=[t,i,one(d.get('problem'),100),progress(d),'●']
        for c,v in enumerate(vals):summary.cell(r,c).text=v
    ...
    if mode=='existing':
        ...
        for sl in prs.slides:
            ...
            if not z:z=key(' '.join(cl.text for _,_,_,cl in table_cells(sl)))
            if kt and ki and kt in z and ki in z:target=sl;break
    if target is None:target=clone(prs,prs.slides[1] if len(prs.slides)>1 else prs.slides[0])
    replace_metadata(target,t,i,g,d);fill_8d_sections(target,d)
    for tb,r,c,cl in table_cells(target):
        txt=norm(cl.text);low=txt.lower()
        if 'signal' in low and c+1<len(tb.columns):signal(tb.cell(r,c+1),st)
        if ('발생 일자' in txt or '발생일자' in txt) and c+1<len(tb.columns):tb.cell(r,c+1).text=one(d.get('occurrence_date'))
        if ('과제명' in txt or '이슈 제목' in txt) and c+1<len(tb.columns) and blank(tb.cell(r,c+1).text):tb.cell(r,c+1).text=t
    Path(out).parent.mkdir(exist_ok=True);saved=save_ppt(prs,out);msg='주간회의 PPT 업데이트: '+st
    if str(saved)!=str(out):msg+=f'\n※ 기존 결과 PPT가 열려 있어 새 파일로 저장: {saved.name}'
    return msg,saved
class App(tk.Tk):
    def __init__(self):
        super().__init__();self.title('8D 이슈 자동화 v2.4');self.geometry('820x700');self.vars={};self.mode=tk.StringVar(value='existing')
        ...
        self.log=tk.Text(self,height=15,width=100);self.log.pack(padx=18,pady=10);ttk.Button(self,text='8D 내용 미리보기',command=self.preview).pack(pady=4);ttk.Button(self,text='자동 업데이트 실행',command=self.run).pack(pady=4)
    def pick(self,k):
